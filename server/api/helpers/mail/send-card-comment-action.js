...
module.exports = {
  ...
  async fn(inputs) {
    const values = _.pick(inputs, ['to', 'name', 'cardId', 'cardName', 'from', 'comment']);
    const info = await transporter.sendMail({
      from: process.env.MAIL_FROM,
      to: values.to,
      subject: `${values.from} adicionou um comentário no card: ${values.cardName}`,
      html: `
      <!DOCTYPE html>
      <html>
        <head>
        ...
        <body>
          ...
          <h1 class="header">Olá, ${values.name}!</h1>
            <div class="content">
              <p><b>${values.from}</b> adicionou o seguinte comentário:</p>
              <br />
              <i>${values.comment}</i>
              <br />
